...
my_input = {"headers": {"authorization": concat(" ", ["Bearer", io.jwt.encode_sign(
	{"alg": "HS256", "typ": "JWT"},
	my_claim,
	{"kty": "oct", "k": secret64},
)])}}
...
test_decode_claims {
	c := claims with input as my_input
	c == my_claim
}

test_decode_claims_username {
	u := username with input as my_input
	u == "B000001"
}

test_decode_claims_roles {
	r := roles with input as my_input
	"role001" in r
}
